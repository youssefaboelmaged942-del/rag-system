...
08_evaluation.py
------------------
Evaluation stage for the RAG pipeline.

This is NOT part of the original 8-file lab sequence (01-07 +
streamlit_app.py) -- it's an additional, separate script that
evaluates the quality of the pipeline you already built.

This version uses REAL GROUND TRUTH: for each test query, the
expected relevant postings below are specific (Title, Company) pairs
that were manually verified to exist in the actual cleaned dataset
(data/Wuzzuf_Jobs.csv + data/DataAnalystJobs.csv) at the time this
file was written. This is stronger than a keyword heuristic --
06_retrieve_context.py's output is checked against postings we KNOW
are in the data and KNOW are relevant, not against a guessed keyword.

Two things are evaluated:

  1) Retrieval evaluation (build_context): for each test query, does
     06_retrieve_context.py's retrieved top-K actually include at
     least one of the manually-verified ground-truth postings?
     (Hit@K, plus Recall@K across the full ground-truth set.)
  2) Answer evaluation: does 07_prompting.py's generated report...
       a) contain all four required Arabic section headers?
       b) stay grounded -- does it reference at least one company or
          job title that is actually present in the retrieved context
          (a lightweight faithfulness check)?

Run directly:
    python 08_evaluation.py

Requires the vector store to already be built (run 01-05 first).
Answer evaluation additionally requires OPENROUTER_API_KEY -- if it
isn't set, retrieval evaluation still runs and answer evaluation is
skipped with a clear message instead of failing.

NOTE ON MAINTENANCE: if you re-scrape or change the source CSVs, the
ground-truth (Title, Company) pairs below may no longer exist in the
data. Re-verify them against data/*.csv before trusting a run.
"""
