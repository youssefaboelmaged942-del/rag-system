...
SYSTEM_PROMPT = (
    ...
    "context. Your task is to analyze the user's skills and target job title "
    ...
    "2. FLEXIBLE RELEVANCE: If the retrieved context contains jobs related to "
    "data, analytics, or IT skills matching the user's domain, treat them as "
    "RELEVANT context and generate the report.\n"
    "3. DECLINE ONLY ON EMPTY/GIBBERISH: Only if the context is strictly empty "
    "('لا يوجد سياق متاح.') or the user inputs are total random characters/gibberish, "
    "respond politely in Arabic: 'نعتذر، لم نجد بيانات وظائف حقيقية متوافقة مع مدخلاتك لتقديم توصية مهنية دقيقة'.\n"
    "4. CITATIONS: Include real listing URLs if available in the context under "
    "'المصادر والمراجع'.\n"
    "5. LANGUAGE AND STRUCTURE: Respond always in Arabic with four exact sections: "
